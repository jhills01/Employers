...
uint32_t subWord(uint32_t word)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        // ensures b captures entire word one byte at a time
        b = word >> i*8; 
...
{
    uint8_t wordsSize = words.size(); 
    uint8_t ff;
    uint32_t tempWord;
    vector<uint32_t> rCon (wordsSize / nk);
    int i;
    
    // initialize rCon vector
    for (i = 0; i < rCon.size(); ++i)
    {
        if (i == 0) ff = 1;
        else xtime(&ff);
    ...
    // initialize words (i.e., the key schedule)
    for (i = 0; i < nk; ++i)
    {
        words[i] = (cipherKey[4 * i] << 24) | (cipherKey[4 * i + 1] << 16) | (cipherKey[4 * i + 2] << 8) | cipherKey[4 * i + 3];
    }

    // build key schedule
    for (i = nk; i < wordsSize; ++i)
    {
        tempWord = words[i - 1];

        if ((i % nk) == 0)
        {
            tempWord = subWord(rotWord(tempWord)) ^ rCon[(i - 1) /nk];
        }
        else if (nk > 6 && (i % nk == 4))
        {
