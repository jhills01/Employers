...
//console.log("camera.position: just after new camera");
...
demo.prototype.init = function()
{
    ...
    //---initialize all textures and associated buffers
    //initTextures(GC, gl);
    initTextures_BMCube(GC, gl);
}
...
//console.log("m.vIndices: ");
//console.log(m.vIndices);
    m.vIndices.forEach(function(d,i)
    {
//console.log("d: " + d); 
//console.log("i: " + i); 
...
//*/
  ...
  m.vIndices.forEach(function(d,i)
  {
      ...
      //NOTE: I had to do this block of code again b/c I need min/max variables for RED calcuation below
      vx = (parseFloat(m.vertices[d*3]));
      vy = (parseFloat(m.vertices[d*3+1]));
      vz = (parseFloat(m.vertices[d*3+2]));
...
//IMPORTANT NOTE: This code is changing camera position from [0,0,-1] default to some bs position
//                For cube and sphere, z becomes ~ +0.8###. I see NO value in this so I'm commenting
//                out for now. NOTE: I don't know if "s" is used elsewhere. Could cause problems.
/*
...
console.log("d value:");
console.log(d);
console.log("camera.position: just after 'd' calculation");
console.log(camera.position);
//*-/
...
function drawScene()
{
//console.log("camera.position: beginning of drawScene");
//console.log(camera.position);
...
//IMPORTANT NOTE: 
//"mv" == modelView in the following functions: i.e., these functions ONLY affect model view matrix!!!
...
//NOTE: Since both cubes are already centered at origin, BOTH mvTranslate lines are not necessary
///*
    ...
    mvMultMatrix(camera.Transform,GC);//multiply by the transformation
                                      //GC.mvMatrix = camera.Transform * GC.mvMatrix

    //translate back to original origin
    mvTranslate([-(m.minX+m.maxX)/2.0,-(m.minY+m.maxY)/2.0,-(m.minZ+m.maxZ)/2.10],GC);
//*/
//---------
    ...
    //---do extra work to include texture information for drawing scene
    //addTexInfoToDrawScene(GC, gl);
    drawScene_BMCube(GC, gl);

    //NOTE: I'm going to call "gl.drawArrays()" in 
    //draw everything
    //gl.drawArrays(gl.TRIANGLES,0,m.indices.1length);
    //gl.drawArrays(gl.TRIANGLES,0,m.vIndices.length);
}
