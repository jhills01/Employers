<?php
    ...
    if (setAndNotEmptyStr($_GET["email"], "email")) {$email = $_GET["email"];}
    if (setAndNotEmptyStr($_GET["password"], "password")) {$password = $_GET["password"];}
        
    // construct query
    // NOTE: "bind_param" was not workng b/c I forgot to put extra space at the end of each string... 
    //       ... for each concatenated line
    $sql = "SELECT s.instructorId, s.sectionId, s.courseId, com.major, s.semester, s.year " .
    "FROM Instructors i " .
