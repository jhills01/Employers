...
main(int argc, char **argv)
{
	...
	unsigned char f_buffer[350000];
	int all_bytes, num_names, place_multiplier, i, j, fbindex; 
	int ip_address[4]; 
	...
	//read entire converted file into f_buffer
	all_bytes = read(file, f_buffer, 350000);

	//start our f_buffer index at -1 to account for 0 indexing
	fbindex = -1;

	//keep traversing f_buffer until 
	while (fbindex < all_bytes)
	{
		...
		{
			++fbindex;
			c->ip_address[i] = f_buffer[fbindex];
		}
		...
		for (i = 0; i < 4; ++i)
		{
			//As long as I don't hit EOF, put together int num_names
		 	++fbindex;
			num_names += place_multiplier * f_buffer[fbindex];
			place_multiplier /= 10;
		...
		for (i = 0; i < num_names; ++i)
		{
			...
			//set f_buffer index to start reading in names
			++fbindex;
		
			//read in names char by char into smaller buffer (yes, I'm lazy and want to reuse code I already had previously)
			while ( f_buffer[fbindex] != 0 )		
			{
				buffer[j] = f_buffer[fbindex];

				++j;
				++fbindex;
			}

			//null terminator string in buffer
			buffer[j] = f_buffer[fbindex];
